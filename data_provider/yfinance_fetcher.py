...
class YfinanceFetcher(BaseFetcher):
    ...
    def _is_th_suffix_stock(stock_code: str) -> bool:
        """Return True for Thai stock symbols (.BK suffix)."""
        return stock_code.strip().upper().endswith('.BK')

    def _is_us_stock(self, stock_code: str) -> bool:
        """
        判断代码是否为美股股票（排除美股指数）。
        支持 US ETFs (如 JEPQ) 和常规美股。
        """
        code = stock_code.strip().upper()
        # 如果是纯字母（ETF或常规美股），直接放行，兼容底层 mapping 可能漏掉的 ETF
        return is_us_stock_code(code) or code.isalpha()
    ...
    def _convert_stock_code(self, stock_code: str) -> str:
        ...
        - 泰股：PTT.BK (Stock Exchange of Thailand)
        - 美股/美股ETF：AAPL, JEPQ (无需后缀)
        ...
        # 日股/韩股/台股/泰股：显式 Yahoo Finance suffix-only 代码，原样传给 Yahoo。
        if self._is_jp_kr_suffix_stock(code) or self._is_tw_suffix_stock(code) or self._is_th_suffix_stock(code):
        ...
        # 已经包含后缀的情况 (增加 .BK 泰股后缀支持)
        if any(suffix in code for suffix in ['.SS', '.SZ', '.HK', '.BJ', '.BK']):
            return code

        # 去除可能的 .SH 后缀
        code = code.replace('.SH', '')

        # ETF: Shanghai ETF (51xx, 52xx, 56xx, 58xx, 50xx) -> .SS; Shenzhen ETF (15xx, 16xx, 18xx) -> .SZ
        if len(code) == 6:
    ...
    def _get_us_index_realtime_quote(
        ...
        """Get realtime quote for US index."""
        import yfinance as yf
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        """获取行情数据，添加了对 Thai 股票的支持及美股ETF保障"""
        import yfinance as yf
        ...
        # 仅处理受支持的市场，现在包含了泰股 (.BK) 和全面美股 (包含ETF)
        if not (
            ...
            logger.debug(f"[Yfinance] {stock_code} 不是受支持的市场代码，跳过")
            return None
